...
- Raspbian as the underlying OS (ref:1). Detailed again later.
- GlusterFS as distributed filesystem (ref:2). I have chosen this based on good experiences in the past. It is simple and works fine on Raspberry Pi.
- Kubernetes as orchestrator (ref:3). Since this officially supports ARM today, it sounds like a good choice.
...
Since there is a sheer infinite number of guides out there how to do a basic SD-Card setup for the RPi, I will not be covering this here (see e.g., (ref:4)). Just one note: If you want to perform a headless setup, don't forget to add a file named "ssh" to the boot partition. 

For higher performance and longer lifetime, I decided to also move the operating system to the external drive (see also (ref:5)).
...
This is fairly simple, you may follow this guide (ref:6) or the following steps. To format the external drive with two partitions with ext4:
```
sudo fdisk /dev/sda
...
While it is possible to use GlusterFS in an extremely configurable way via REST, e.g., through the use of heketi (ref:7). I decided against using such a setup, for two reasons: Firstly, heketi needs at least a replication of three, which does not work well with two devices. Secondly, it seems to be over-complicating my setup, as I might only need some simple volumes.

Now, we can prepare for GlusterFS (see e.g., (ref:8), and official documentation (ref:9)). First, make sure that the second partition on the SSD is always mounted:
```
sudo mkdir /mnt/ssd2
...
I rely on some guides specific to Raspberry Pi (e.g., (ref:10)) and also heavily on the excellent Kubernetes documentation, which I can only recommend to refer to whenever you are in doubt (ref:11).
...
Kubernetes can be used with different network implementations (see also (ref:12)). Reasonable choices for the RPi are Flannel and Weave Net. I will be using the latter. On node one run:
```
...
Done! That was surprisingly easy! In my case everything was rather straight-forward. Depending on your setup, you might experience a hiccup here and there. In that case, refer to the Kubernetes documentation (see (ref:13)).
We now have a running Kubernetes cluster on Raspberry Pi!

(l3: Kubernetes Dashboard)
...
We will be using the API Server to access the dashboard. For this to work, we need to install client certificates in the browser (see (ref:14)). We will be extracting the certificates from kubeconfig for this purpose (see (ref:15)):
```
...
However, as you can see, we need a token to access the dashboard. So lets generate this token now (see also (ref:16)).
...
(l2: References)
(source: 1 url:https://www.raspberrypi.org/downloads/raspbian/)
(source: 2 url:https://www.gluster.org/)
(source: 3 url:https://kubernetes.io/)
(source: 4 url:https://projects.raspberrypi.org/en/projects/raspberry-pi-setting-up)
(source: 5 url:https://jamesachambers.com/raspberry-pi-3b-microsd-vs-ssd-speed-benchmarks/)
(source: 6 url:https://www.tomshardware.com/news/boot-raspberry-pi-from-usb,39782.html)
(source: 7 url:https://github.com/heketi/heketi)
(source: 8 url:https://www.howtoforge.com/tutorial/high-availability-storage-with-glusterfs-on-ubuntu-1804/)
(source: 9 url:https://docs.gluster.org/en/latest/)
(source: 10 url:https://kubernetes.io/docs/setup/production-environment/tools/kubeadm/install-kubeadm/)
(source: 11 url:https://medium.com/nycdev/k8s-on-pi-9cc14843d43)
(source: 12 url:https://kubernetes.io/docs/concepts/cluster-administration/networking/)
(source: 13 url:https://kubernetes.io/docs/home/)
(source: 14 url:https://github.com/kubernetes/dashboard/blob/master/docs/user/accessing-dashboard/1.7.x-and-above.md)
(source: 15 url:https://www.australtech.net/kubernetes-unable-to-login-to-the-dashboard/)
(source: 16 url:https://medium.com/@kanrangsan/creating-admin-user-to-access-kubernetes-dashboard-723d6c9764e4)
(source: 17 url:https://kubernetes.io/docs/concepts/storage/volumes/#glusterfs)
(source: 18 url:https://github.com/kubernetes/examples/tree/master/volumes/glusterfs)
(source: 19 url:https://medium.com/velotio-perspectives/demystifying-high-availability-in-kubernetes-using-kubeadm-3d83ed8c458b)
