...
Tags: work,TUM CREATE,cluster,tech
...
In my current case, each testcase is a (link: http://www.prismmodelchecker.org/ text: PRISM) computation, which runs as a single thread, thus running on only one CPU core. The computations utilize this CPU core at 100%. As my workstation in the office has a quad core processor with Hyper-Threading, I have effectively 8 cores available to run these tasks in parallel. When the runtime of testcases increases to something like 24h, 8 cores can still be quite a bottleneck, when computing a higher number of testcases (e.g. 20). To speed up these computations, I implemented a small computing cluster, by utilizing all workstations with Linux-based operating systems that we have available at work.
...
(toc: 5)
(l2: Installation)
We use (link: http://www.adaptivecomputing.com/products/open-source/torque/ text: TORQUE) to connect our nodes. Specifically we use TORQUE version 4.2.9. This is available via Arch Linux repositories or on the link above for systems with older package managers. We use e.g. also Ubuntu systems, whose package managers only offer version 2.4.16. 
...
2. DDNS
>If IP addresses are dynamic, you might want to use a dynamic DNS (DDNS) service. This way, you can sync your IP addresses to a host name, which you can in turn use to connect TORQUE. DDNS can be efficiently implemented with ddclient and (link: http://freedns.afraid.org/ text: FreeDNS popup: yes). A quick Google search should show you how to set this up. You want to use the interface IP for synchronization in ddclient, not the external IP.
>
...
Following the setup, as described in the (link: https://wiki.archlinux.org/index.php/TORQUE text: ArchLinux Wiki popup: yes), we use the following server settings:
```
...
Check out the full references of (link: http://gridscheduler.sourceforge.net/htmlman/htmlman1/qsub.html text: qsub) and (link: http://gridscheduler.sourceforge.net/htmlman/htmlman1/qstat.html text: qstat) for all options. A good tutorial on the basic usage is available at (link: https://wikis.nyu.edu/display/NYUHPC/Tutorial+-+Submitting+a+job+using+qsub text: NYU).
